...
BEGIN
    ...
    raw_data AS (
        SELECT
            ...
            uc.sex_code,
            CASE
                WHEN uc.sex_code = 'M' THEN task_6.pick_lookup_value('fname_male', p_locale, uc.u_seed || '_fn')
                ELSE task_6.pick_lookup_value('fname_female', p_locale, uc.u_seed || '_fn')
            END as first_name,
            CASE
                WHEN task_6.get_deterministic_float(uc.u_seed || '_has_mn') < 0.35 THEN
                    CASE
                        WHEN uc.sex_code = 'M' THEN task_6.pick_lookup_value('middlename_male', p_locale, uc.u_seed || '_mn')
                        ELSE task_6.pick_lookup_value('middlename_female', p_locale, uc.u_seed || '_mn')
                    END
                END as middle_name,
            task_6.pick_lookup_value('last_name', p_locale, uc.u_seed || '_ln') as last_name,
            CASE
                WHEN task_6.get_deterministic_float(uc.u_seed || '_has_title') < 0.2 THEN
                    CASE
                        WHEN uc.sex_code = 'M' THEN task_6.pick_lookup_value('title_male', p_locale, uc.u_seed || '_tl')
                        ELSE task_6.pick_lookup_value('title_female', p_locale, uc.u_seed || '_tl')
                    END
                END as title,
            task_6.pick_lookup_value('city', p_locale, uc.u_seed || '_city') as city,
            task_6.pick_lookup_value('street', p_locale, uc.u_seed || '_st') as street,
            task_6.pick_lookup_value('email_domain', p_locale, uc.u_seed || '_em') as email_domain,
            task_6.pick_lookup_value('eye_color', p_locale, uc.u_seed || '_eye') as eyes,
            task_6.pick_lookup_value('hair_color', p_locale, uc.u_seed || '_hair') as hair,
            task_6.pick_lookup_value('skin_tone', p_locale, uc.u_seed || '_skin') as skin,
            task_6.pick_lookup_value('build', p_locale, uc.u_seed || '_build') as body_build
        FROM user_config uc
    )
    SELECT
        ...
        task_6.get_random_formatted('phone_format', p_locale, rd.u_seed || '_ph') as phone,

        CONCAT(
            rd.street, ' ',
            floor(task_6.get_deterministic_float(rd.u_seed || '_hn') * 200 + 1)::int, ', ',
            task_6.get_random_formatted('postcode_format', p_locale, rd.u_seed || '_pc'), ' ',
            rd.city
        ) as address_full,
